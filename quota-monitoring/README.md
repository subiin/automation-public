...
1.프로젝트 ID와 region 입력
  - blueprints > cloud-operations > quota-monitoring > variables.tf의 project_id와 region에 각각 프로젝트 ID, region 입력
    
2.threshold_value 조정
  - blueprints > cloud-operations > quota-monitoring > main.tf의 각 alert resource에 지정된 default threshold_value를 프로젝트 상황에 맞게 조정
    
3.Cloud Scheduler Frequency 조정
  - blueprints > cloud-operations > quota-monitoring > variables.tf의 schedule_config에 실행될 Frequency 조정
    
4.Terraform 실행
  - blueprints/cloud-operations/quota-monitoring로 디렉토리 이동
  - terraform init
  - terraform apply -var project_id=[project_id]
    
5.생성된 리소스 확인
  - "실행 시 생성되는 리소스 목록" 참고하여 리소스 생성 확인
  - Alerting Policy 활성화
...
- 모니터링 항목 추가 시 아래의 절차 필요
 - Metrics explorer > Resource & Metric에서 Consumer Quota 입력 후 Metric 선택
   - Metric은 [링크](https://cloud.google.com/docs/quota#types_of_quota) 참고
- Filters에서 추가하고 싶은 항목의 quota_metric을 필터링
- 그래프로 추이 확인 후 alert을 받을 threshold 값 정하기 
- main.tf에서 기존 리소스를 복사하여 값 변경
 - Reference name, display_name, threshold_value, content
 - filter는 아래의 형태로 변경
